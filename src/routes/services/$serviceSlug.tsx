...
function ServiceBookingForm({ serviceName }: { serviceName: string }) {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try { await createLead({ name: String(data.get("name")), phone: String(data.get("phone")), postalCode: String(data.get("postalCode")), service: serviceName, source: "service" }); event.currentTarget.reset(); setMessage("Thanks — your request has been sent."); }
