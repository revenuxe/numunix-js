  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <button onClick={() => void loadLeads()} className="text-sm font-semibold text-brand">
          Refresh
        </button>
      </div>
      {error && (
      ...
      {loading ? (
        ...
        <div className="grid gap-3">
          {leads.map((lead) => (
            <article
              key={lead.id}
              className="rounded-2xl bg-white p-4 shadow-soft ring-1 ring-border sm:flex sm:items-center sm:justify-between sm:gap-4"
            >
              <div className="min-w-0">
                ...
                <p className="mt-1 truncate text-sm text-muted-foreground">
                  {lead.phone || lead.email || "No mobile number"}
                </p>
                <p className="mt-1 font-mono text-xs font-semibold text-brand">
                  Booking: {lead.booking_id || lead.id}
                </p>
              ...
              <div className="mt-4 flex gap-2 sm:mt-0">
                <button
                  ...
                  className="grid h-10 w-10 place-items-center rounded-xl bg-secondary text-ink transition hover:bg-brand hover:text-brand-foreground"
                >
                ...
                <button
                  ...
                  className="grid h-10 w-10 place-items-center rounded-xl bg-red-50 text-red-700 transition hover:bg-red-600 hover:text-white"
                >
      ...
      {selected && (
        <div
          ...
          aria-modal="true"
        >
          <section className="w-full max-w-sm rounded-[2rem] bg-white p-6 shadow-card">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[.2em] text-brand">
                  Lead details
                </p>
                <h2 className="mt-1 text-2xl font-extrabold">{selected.name}</h2>
              </div>
              <button
            ...
            <dl className="mt-6 space-y-4 text-sm">
              <Detail label="Booking reference" value={selected.booking_id || selected.id} />
              <Detail label="Service" value={selected.service} />
              <Detail label="Mobile number" value={selected.phone} />
            ...
            <button
              ...
              className="mt-7 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-red-600 px-4 py-3 text-sm font-semibold text-white"
            >
              <Trash2 className="h-4 w-4" />
              Delete lead
            </button>
