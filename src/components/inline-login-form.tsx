...
export function InlineLoginForm({
  ...
  async function onSubmit(event: React.FormEvent) {
    ...
    if (data.session) {
      ...
      // there the first time they book a pickup — never blocks continuing.
      try {
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="w-full rounded-[1.75rem] bg-ink p-6 text-white shadow-card ring-1 ring-white/10 sm:p-7"
    >
      ...
      <h2 className="mt-1.5 text-2xl font-extrabold">
        {mode === "sign-in" ? (title ?? "Sign in to continue") : "Create your account"}
      </h2>
      <p className="mt-1.5 text-sm text-white/70">
        {mode === "sign-in"
          ? (description ?? "Sign in to book a free pickup for your device.")
          : "We'll use these details to book your pickup."}
      </p>
